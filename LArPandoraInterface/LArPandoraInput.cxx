...
std::cout << "fifth cout" <<std::endl;
    ...
    for (MCParticleMap::const_iterator iterI = particleMap.begin(), iterEndI = particleMap.end(); iterI != iterEndI; ++iterI)
    {
	    ...
	    mcParticleParameters.m_Process = particle->Process();
	    mcParticleParameters.m_endProcess = particle->EndProcess();

	    if(nuanceCode==2001||nuanceCode==2000)
	    {
	        std::cout << "Nuance code " <<nuanceCode << " momentum x" <<  mcParticleParameters.m_momentum.Get().GetX()<< " momentum y" <<  mcParticleParameters.m_momentum.Get().GetY() << " momentum z" <<  mcParticleParameters.m_momentum.Get().GetZ() <<std::endl;      
                std::cout << " Process is " << particle->Process() << " and end process is " << particle->EndProcess() <<std::endl;
           }
	}
        catch (const pandora::StatusCodeException &)
        {
